...
public class VesselTrackingEndpointTests : IClassFixture<WebApplicationFactory<Program>>, IDisposable
{
    ...
    public async Task CreateRun_WithAVideo_DetectsOnItsPicturesAndBindsTheVessel()
    {
        ...
        var body = await response.Content.ReadFromJsonAsync<VesselTrackingRunResponse>();
        var frame = Assert.Single(body!.Frames);
        Assert.Equal(431234567, Assert.Single(frame.FusedTracks).Mmsi);
    ...
    private HttpClient ClientWithVideo(
        IVideoFrameRepository videoFrameReader,
        IVesselDetector? detector,
    ...
    private sealed class FakeVideoFrameReader : IVideoFrameRepository
    {
    ...
    // Reports one vessel-sized box at a fixed spot on every picture.
    private sealed class FakeDetector(double centreX, double centreY) : IVesselDetector
    {
        public IReadOnlyList<DetectionBox> Detect(FrameImage image, DateTimeOffset timestamp) =>
            [new DetectionBox(centreX - 30, centreY - 20, centreX + 30, centreY + 20, timestamp)];
    }
