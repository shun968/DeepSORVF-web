...
namespace LayeredArchitecture.Infrastructure.Detection;
...
//
// Registered once: the model is loaded on the first picture, and ONNX Runtime allows concurrent
// runs on one session.
public sealed class YoloxVesselDetector : IVesselDetector, IDisposable
{
    ...
    public YoloxVesselDetector(string modelPath)
    {
    ...
    public IReadOnlyList<DetectionBox> Detect(FrameImage image, DateTimeOffset timestamp)
    {
    ...
    private static List<DetectionBox> ToDetections(Tensor<float> rows, Letterbox letterbox, FrameImage image, DateTimeOffset timestamp)
    {
